...
fn run_command(cmd_line: &[u8]) {
    ...
    if pipe_idx < cmd_line.len() {
        ...
        if left_pid == 0 {
            /* Child: set stdout to pipe write end */
            /* Close read end, dup write end to fd 1 */
            sys_pipe_close(fds[0]);
            /* We need dup2, for now write to pipe fd directly */
            /* Actually the program writes to stdout (sys_write) */
            /* We'd need a way to redirect. For now, skip pipe. */
            sys_pipe_close(fds[1]);
            sys_exit_code(1);
        }
