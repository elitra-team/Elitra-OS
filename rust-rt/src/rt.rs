...
pub fn sys_open(path: &str) -> isize {
    let r = unsafe { syscall_2(4, path.as_ptr() as u32, path.len() as u32) };
...
pub unsafe extern "C" fn realloc(ptr: *mut u8, size: usize) -> *mut u8 {
    ...
    if ptr == LAST_ALLOC_PTR {
        let aligned = (size + 7) & !7;
        let old_aligned = (LAST_ALLOC_SIZE + 7) & !7;
    ...
    if !newp.is_null() && newp != ptr {
        // Copy as much as safely known (LAST_ALLOC_SIZE for the last alloc)
        let known_size = if ptr == LAST_ALLOC_PTR { LAST_ALLOC_SIZE } else { size };
        let copy_size = if known_size < size { known_size } else { size };
